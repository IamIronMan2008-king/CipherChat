...
USERS_DB_PATH = os.environ.get('USERS_DB_PATH', 'users_db.json')
...
def register_user(user_id, name, password):
    """Attempts to create a new user. Returns (True, None) on success or
...
def authenticate_user(user_id, password):
    """Returns (True, None) if the id/password pair is valid, otherwise
    (False, reason_code)."""
    with db_lock:
        users = _load_users()
    ...
    while True:
        ...
        if data.startswith(REGISTER_PREFIX):
            ...
            if not user_id or not password:
                _send_raw(client, AUTH_FAIL_PREFIX + REASON_BAD_REQUEST)
                continue
            success, reason = register_user(user_id, name, password)
            if success:
                _send_raw(client, AUTH_OK_PREFIX + user_id)
                return user_id
            _send_raw(client, AUTH_FAIL_PREFIX + reason)
            continue
        ...
        if data.startswith(LOGIN_PREFIX):
            ...
            success, reason = authenticate_user(user_id, password)
            if success:
                _send_raw(client, AUTH_OK_PREFIX + user_id)
                return user_id
            _send_raw(client, AUTH_FAIL_PREFIX + reason)
